...
export function buildApi(
  ...
  void app.register(async (router) => {
    ...
    router.get('/a/:reference', async (request, reply) => {
      ...
      const connected = card.verification?.status === 'verified' && card.transports.length > 0;
      const presence = connected ? 'connected' : (card.presence?.status ?? 'offline');
