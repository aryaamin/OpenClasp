...
const allowedScopes = new Set(['mcp:access']);
...
export async function authorize(request: Request, store: OAuthStore): Promise<Response> {
  ...
  const requestedScopes = (url.searchParams.get('scope') ?? 'mcp:access')
    .split(/\s+/)
    .filter(Boolean);
  if (!requestedScopes.length || requestedScopes.some((scope) => !allowedScopes.has(scope)))
    return oauthError('invalid_scope', 'Only mcp:access is supported');
